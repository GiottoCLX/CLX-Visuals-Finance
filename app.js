...
(() => {
  const sb = window.supabase.createClient(window.__SUPABASE__.url, window.__SUPABASE__.anonKey);
  ...
  // ===== Router über URL-Hash
  const views = [...document.querySelectorAll('.view')];
  const viewTitle = document.querySelector('#view-title');

  function setActiveByHash(hash){
    document.querySelectorAll('.side-btn[data-go]').forEach(a=>{
  ...
  // ===== Mobile Drawer
  const sidebar = qs('#sidebar'), backdrop = qs('#side-backdrop'), btnNav = qs('#btn-nav');
  ...
  // ===== Auth
  const auth = qs('#auth'), authEmail=qs('#auth-email'), authPw=qs('#auth-password'), authMsg=qs('#auth-msg');
  const btnSignup=qs('#btn-signup'), btnSignin=qs('#btn-signin'), btnSignout=qs('#btn-signout'), userEmail=qs('#user-email');
  ...
  sb.auth.onAuthStateChange(async (_e, session) => {
    ...
    if (!user){ auth.classList.add('show'); userEmail.textContent=''; btnSignout.style.display='none'; return; }
    auth.classList.remove('show'); userEmail.textContent=user.email; btnSignout.style.display='inline-block';
    await ensureDefaults(user.id);
    setDefaultDates();
    await Promise.all([loadAccounts(), loadCategories()]);
    reloadAll();
  });
  (async()=>{ const {data:{session}}=await sb.auth.getSession(); if(session?.user){ auth.classList.remove('show'); userEmail.textContent=session.user.email; btnSignout.style.display='inline-block'; await ensureDefaults(session.user.id); setDefaultDates(); await Promise.all([loadAccounts(), loadCategories()]); reloadAll(); } else { auth.classList.add('show'); } })();

  // ===== Filter
  const fMonth=qs('#f-month'), fAcc=qs('#f-account'), fCat=qs('#f-category'), search=qs('#f-search');
  const btnRefresh=qs('#btn-refresh'), btnClear=qs('#btn-clear');
  qsa('.chip[data-qf]').forEach(ch=>ch.onclick=()=>applyQuickFilter(ch.dataset.qf));
  function setDefaultDates(){ const d=new Date(); fMonth.value=d.toISOString().slice(0,7); const df=qs('input[name="date"]'); if(df) df.value=d.toISOString().slice(0,10); }
  function applyQuickFilter(k){ const now=new Date(), end=new Date(); if(k==='this_month'){ fMonth.value=now.toISOString().slice(0,7); delete search.dataset.range; } if(k==='last_month'){ now.setMonth(now.getMonth()-1); fMonth.value=now.toISOString().slice(0,7); delete search.dataset.range; } if(k==='7d'){ fMonth.value=''; const s=new Date(); s.setDate(end.getDate()-6); search.dataset.range=`${s.toISOString().slice(0,10)},${end.toISOString().slice(0,10)}`; } if(k==='all'){ fMonth.value=''; delete search.dataset.range; } reloadAll(); }
  ...
  // ===== Accounts/Categories
  const txAcc=qs('#tx-account'), txCat=qs('#tx-category'), accList=qs('#acc-list'), catList=qs('#cat-list');
  async function ensureDefaults(user_id){
    const {data:a}=await sb.from('accounts').select('id').limit(1); if(!a||!a.length) await sb.from('accounts').insert([{user_id,name:'Bargeld'},{user_id,name:'Bank'}]);
    const {data:c}=await sb.from('categories').select('id').limit(1); if(!c||!c.length) await sb.from('categories').insert([{user_id,name:'Essen'},{user_id,name:'Transport'},{user_id,name:'Lohn'},{user_id,name:'Allgemein'}]);
  }
  async function loadAccounts(){
    ...
    const rAcc=qs('#r-account'); if(rAcc) rAcc.innerHTML = txAcc?.innerHTML || '';
  }
  async function loadCategories(){
    ...
    const rCat=qs('#r-category'); if(rCat) rCat.innerHTML = txCat?.innerHTML || '';
  }
  document.addEventListener('click', async (e)=>{
    const a=e.target?.getAttribute?.('data-del-acc'); const c=e.target?.getAttribute?.('data-del-cat');
  ...
  qs('#acc-form')?.addEventListener('submit', async e=>{
    e.preventDefault(); const name=new FormData(e.target).get('name').toString().trim();
  ...
  // ===== Transactions
  const txBody=qs('#tx-table tbody'), txBodyDash=qs('#tx-table-dash tbody');
  const pagePrev=qs('#page-prev'), pageNext=qs('#page-next'), pageInfo=qs('#page-info'), pageSizeSel=qs('#page-size');
  ...
  function getKey(r,k){
    if(k==='amount') return r.amount*(r.is_income?1:-1);
    if(k==='type') return r.is_income?1:0;
    if(k==='account') return (r.accounts?.name||'').toLowerCase();
    if(k==='category') return (r.categories?.name||'').toLowerCase();
    return (r[k]||'');
  }
  ...
  function renderTable(){
    ...
    txBody.innerHTML=rows.map(r=>{
      const net=r.is_income?+r.amount:-r.amount; bal+=net;
      return `<tr><td>${r.date}</td><td>${esc(r.description||'')}</td><td>${esc(r.accounts?.name||'—')}</td><td>${esc(r.categories?.name||'—')}</td><td class="num">${r.is_income?'Ein':'Aus'}</td><td class="num">${net.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</td><td class="num">${bal.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</td><td class="num"><button class="btn btn-secondary" data-del="${r.id}">×</button></td></tr>`;
    }).join('');
    const max=Math.max(1,Math.ceil(rowsCache.length/pageSize));
    if(pageInfo) pageInfo.textContent=`Seite ${page}/${max} – ${rowsCache.length} Einträge`;
  }
  ...
  qs('#file-import')?.addEventListener('change', async e=>{
    ...
    for(let i=0;i<batch.length;i+=200){ const {error}=await sb.from('transactions').insert(batch.slice(i,i+200)); if(error) return alert(error.message); }
  ...
  // KPIs + Charts + Sidebar
  const kpiIncome=qs('#kpi-income'), kpiExpense=qs('#kpi-expense'), kpiBalance=qs('#kpi-balance'), kpiCount=qs('#kpi-count');
  ...
  // ===== Budgets
  const bMonth=qs('#b-month'), bCat=qs('#b-category'), bAmount=qs('#b-amount'), bForm=qs('#budget-form'), bList=qs('#budget-list'), bHint=qs('#budgets-hint');
  ...
  // ===== Recurring
  const rStart=qs('#r-start'), rEvery=qs('#r-every'), rDesc=qs('#r-desc'), rAmount=qs('#r-amount'), rAcc=qs('#r-account'), rCat=qs('#r-category');
  const btnRecSave=qs('#btn-rec-save'), btnRecApply=qs('#btn-rec-apply'), recList=qs('#rec-list'), recHint=qs('#rec-hint');
    ...
    try{
      ...
      for(const r of recs||[]){
        if(r.every==='monthly'){
          ...
          let cur=new Date(s); while(cur<e){ ins.push({user_id:ses.user.id,date:cur.toISOString().slice(0,10),description:r.description,amount:r.amount,is_income:r.is_income,account_id:r.account_id,category_id:r.category_id}); cur.setDate(cur.getDate()+7); }
        }
      }
  ...
  document.addEventListener('click', async (e)=>{
    ...
    await sb.from('recurring').delete().eq('id',id); loadRecurring();
  });

  // ===== Settings
  const settingsEmail=qs('#settings-email'), btnLight=qs('#theme-light'), btnDark=qs('#theme-dark'), selCur=qs('#currency');
  function refreshSettings(){ if(settingsEmail) settingsEmail.textContent = document.querySelector('#user-email').textContent || '—'; const cur=localStorage.getItem('clx.currency')||'CHF'; if(selCur) selCur.value=cur; }
